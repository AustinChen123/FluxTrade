...
def _corrupt_position(method: str) -> EndpointPosition:
    position = EndpointPosition.model_validate(_position_values())
    if method == "model_copy":
        return position.model_copy(update={"quantity": 1.0})
    return EndpointPosition.model_construct(**{**_position_values(), "quantity": 1.0})


def _corrupt_order(method: str) -> EndpointOrder:
    order = EndpointOrder.model_validate(_order_values())
    if method == "model_copy":
...
@pytest.mark.parametrize(
    ("field", "corrupt"),
    [("positions", _corrupt_position), ("working_orders", _corrupt_order)],
)
@pytest.mark.parametrize("boundary", ["state", "builder"])
def test_endpoint_boundaries_revalidate_exact_nested_instance_content(
    method: str,
    field: str,
    corrupt: Callable[[str], EndpointPosition | EndpointOrder],
    boundary: str,
) -> None:
    instance = corrupt(method)
    if isinstance(instance, EndpointPosition):
        adversarial_value = instance.quantity
    else:
        adversarial_value = instance.price
    assert isinstance(adversarial_value, float)

    with pytest.raises(ValidationError):
        if boundary == "state":
            ReplayEndpointState.model_validate({field: (instance,)})
        else:
            build_replay_endpoint_state(
                positions=(instance,) if field == "positions" else (),
                working_orders=(instance,) if field == "working_orders" else (),
            )
...
def test_builder_rejects_noncanonical_final_mark(invalid: object) -> None:
    arguments = inspect.signature(build_replay_endpoint_state).bind(
        positions=(), working_orders=(), final_mark=invalid, end_timestamp=100
    )
    with pytest.raises(ValueError, match="decimal value"):
        build_replay_endpoint_state(*arguments.args, **arguments.kwargs)
