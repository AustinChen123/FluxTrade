...
## Registering Strategies with StrategyEngine

In live trading, strategies are registered with the `StrategyEngine`:

```python
from src.core.engine import StrategyEngine

engine = StrategyEngine(db_session, clock)
...
# Start the engine (begins processing market data)
engine.startup()
```

For backtesting, use `BacktestRunner.add_strategy()` instead -- see the [Backtesting Guide](backtesting.md).
