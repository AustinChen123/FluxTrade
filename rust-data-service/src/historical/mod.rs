...
pub async fn run_backfill(
    ...
    let downloader: Arc<dyn HistoricalDownloader + Send + Sync> =
    ...
    let now = Utc::now();
    let first_day_current_month = NaiveDate::from_ymd_opt(now.year(), now.month(), 1)
        .ok_or_else(|| anyhow!("Invalid current month date"))?;
    let s3_boundary_ts = Utc
        .from_utc_datetime(
    ...
    if exchange.to_lowercase() == "binance" && start_ts < s3_boundary_ts {
        ...
        info!(
            "Starting S3 backfill from {} to {} ({})",
            start_ts, s3_end, timeframe
        );

        let curr_month_start = NaiveDate::from_ymd_opt(start.year(), start.month(), 1)
            .ok_or_else(|| anyhow!("Invalid start month date"))?;

        let mut s3_tasks = Vec::new();

        // We want to cover all months from start_ts up to s3_end
        let s3_end_dt = if s3_end == s3_boundary_ts - 1 {
            let prev_month = first_day_current_month
                .pred_opt()
                .unwrap_or(first_day_current_month);
            ...
            Utc.timestamp_millis_opt(s3_end)
                .single()
                .ok_or_else(|| anyhow!("Invalid S3 end timestamp: {}", s3_end))?
        };
        let s3_end_date = NaiveDate::from_ymd_opt(s3_end_dt.year(), s3_end_dt.month(), 1)
            .ok_or_else(|| anyhow!("Invalid S3 end date"))?;
        ...
        for task in s3_tasks {
            match task.await? {
                Ok(count) => {
                    if count > 0 {
                        info!("S3 Month finished. Inserted {} candles", count)
                    }
                }
    ...
    if rest_start < end_ts {
        info!(
            "Starting REST backfill from {} to {} ({})",
            rest_start, end_ts, timeframe
        );
...
async fn process_s3_month(
    ...
    let mut candles = downloader
        .fetch_from_s3(&symbol, year, month, timeframe)
        .await?;
    if candles.is_empty() {
        return Ok(0);
    }
...
async fn insert_candles(
    ...
    for curr in candles.iter().skip(1) {
        let (prev_close, last_ts) = match filled_candles.last() {
            Some(last) => (last.close, last.timestamp),
            None => return Err(anyhow!("Empty candle buffer in gap fill")),
        };
...
fn parse_date(date_str: &str) -> Result<DateTime<Utc>> {
    ...
    if let Ok(date) = NaiveDate::parse_from_str(date_str, "%Y-%m-%d") {
        let dt = date
            .and_hms_opt(0, 0, 0)
            .ok_or_else(|| anyhow!("Invalid midnight time for {}", date_str))?;
    ...
    Err(anyhow!(
        "Invalid date format: '{}'. Expected YYYY-MM-DD, YYYY/MM/DD, or Timestamp(ms).",
        date_str
    ))
}
...
impl HistoricalDownloader for BackpackDownloader {
    async fn fetch_candles(
        ...
        for k in raw_data {
            ...
            let volume =
                Decimal::from_str(k.get("volume").and_then(|v| v.as_str()).unwrap_or("0"))?;
