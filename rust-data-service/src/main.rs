        ...
        match ex.trim().to_lowercase().as_str() {
            ...
            "backpack" => {
                tokio::spawn(async move {
                    ...
                    if std::env::var("EXCHANGE_API_KEY").is_ok()
                        && std::env::var("EXCHANGE_SECRET").is_ok()
                    {
                        if let Err(e) = conn.subscribe_user_stream(user_tx).await {
                ...
                if let Some(event) = msg {
                    ...
                    match event {
                        ...
                        UserStreamEvent::Account(update) => {
                             ...
                             if let Err(e) = pub_lock.update_account_balance(&update).await {
                        ...
                        UserStreamEvent::Position(update) => {
                            ...
                            if let Err(e) = pub_lock.update_position(&update).await {
